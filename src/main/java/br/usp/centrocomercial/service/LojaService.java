...
public class LojaService {
	...
	private String queryLojas = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> "+
					            ...
					            "SELECT ?about ?nome ?site ?telefone ?atividade { "+
	...
	private String queryOrder = "ORDER BY ?nome";
	...
	public List<Loja> buscarLojasOrdenado() {
		ResultSet resultSet = repository.executeSelect(String.format(this.queryLojas, "", this.queryOrder));
		List<String> jsonList = jsonConverter.convertResultSetToJson(resultSet);
		List<Loja> response = new Gson().fromJson(jsonList.toString(), lLojaType);
		return response;
